...
type InspectRenterRouteProp = RouteProp<
  { InspectRenter: { user: any } },
  'InspectRenter'
>;

const InspectRenter = () => {
  const route = useRoute<InspectRenterRouteProp>();
  ...
  return (
    ...
    <ScrollView>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Currently Borrowed Listings</Text>
        {(user.currentlyBorrowedList?.length ?? 0) > 0 ? (
          user.currentlyBorrowedList.map((item: any) => (
          ...
          <Text style={styles.empty}>No current borrowed items.</Text>
        )}
      </View>

      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Previously Borrowed Listings</Text>
        {(user.previouslyBorrowedList?.length ?? 0) > 0 ? (
          user.previouslyBorrowedList.map((item: any) => (
            <View key={item.id} style={styles.card}>
              <Text style={styles.itemTitle}>{item.name}</Text>
              <Text>₱{item.price}/{item.plan}</Text>
              {item.description && <Text>{item.description}</Text>}
          ...
          <Text style={styles.empty}>No previous borrowed items.</Text>
        )}
      </View>
    </ScrollView>
    <BottomSpacer />
...
const styles = StyleSheet.create({
  ...
  initials: { fontSize: 30, color: '#666' },
  email: { fontSize: 18, fontWeight: 'bold', textAlign: 'center' },
  role: { fontSize: 14, color: '#666', textAlign: 'center' },
  ...
  empty: { fontStyle: 'italic', color: '#999', textAlign: 'center' },
  input: {
    borderWidth: 1,
    borderColor: '#ccc',
    borderRadius: 6,
  ...
  adminActions: { width: '100%', paddingVertical: 10 },
  button: {
    padding: 12,
    borderRadius: 6,
    alignItems: 'center',
  ...
  error: { color: 'red', fontSize: 12, marginTop: -4, textAlign: 'center' },
  backButton: {
    marginBottom: 16,
    padding: 12,
    backgroundColor: '#ddd',
  ...
  passwordSection: { width: '100%', alignItems: 'center' },
});